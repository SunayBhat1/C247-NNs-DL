...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # Softmax Loss Calc
    softmax_loss = np.sum(np.log(np.sum(np.exp(scores), axis = 1)) - scores[np.arange(N), y])
    # L2 regularization
    l2_reg = 0.5 * reg * (np.linalg.norm(W1)**2 + np.linalg.norm(W2)**2)
    # Final Loss
    loss = softmax_loss / N + l2_reg
    ...
    # Used denominator layout 
    # Loss dL/dz2
    dL_dz2 = np.exp(scores) / np.sum(np.exp(scores), axis=1, keepdims=True)
    dL_dz2[np.arange(N), y] -= 1
    dL_dz2 = dL_dz2/N # normalize by samples
    # Loss dL/da1 = dz2/da1 * dL/dz2
    dL_da1 = (W2.T @ dL_dz2.T).T
    # Loss dL/dz1 = da1/dz1 * dL/da1
    dL_dz1 = (z1 > 0) * dL_da1

    grads['W2'] = dL_dz2.T @ a1 + (reg * W2)     # dL/dW2
    grads['b2'] = np.sum(dL_dz2,axis=0)                 # dL/db2

    grads['W1'] = (X.T @ dL_dz1).T + (reg * W1)      # dL/dW1
    grads['b1'] = np.sum(dL_dz1,axis=0)                 # dL/db1
  ...
  def predict(self, X):
    ...
    # Forward prop and argmax of scores
